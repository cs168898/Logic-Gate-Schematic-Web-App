import { Stage, Layer, Path, Text, Line, Circle } from 'react-konva';
import { gridSizeConst } from '../utils/gridSize';
import React, { useEffect, useRef } from 'react';

export function NandGate({ gate, selectedGateId, setSelectedGateId, onWirePositionUpdate }){
    const andGatePath = `
          M 75,50 
          Q 75,0 0,0 
          Q 0,25 0,50 
    ...
    const circleX = gate.x *gridSizeConst + (gridSizeConst * 4); // X position (a bit outside of the semicircle)
    const circleY = gate.y *gridSizeConst + (gridSizeConst * 2.5);  // Y position (aligned with the middle of the semicricle)
